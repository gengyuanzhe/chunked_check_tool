...
- 观测指标（无 S3 LIST，不显示 `list_*`）：进度行 `[progress] read=X list_failed=… backup_ok=… backup_failed=… backup_mismatch=… backup_skipped_clean=… get_calls=… get_avg_ms=… (backed=N) q=obj:… lf:… bok:… bfail:… mm:… bsc:…`；汇总 `read: X`（累计已读行数，含坏行，不统计总行数）+ `list_failed` + `get_calls`（多段校验探测产生）+ `backup_ok: N backup_failed: N backup_mismatch: N backup_skipped_clean: N`（`backup_mismatch` = 输入类型校验失败数，见流程第 3 步）
